...
public class AuthenticationFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		Optional<String> token = Optional.ofNullable(httpRequest.getHeader("Bearer"));
	...
	private void processUserAuthentication(HttpServletResponse httpResponse, User user) throws IOException {
		Authentication resultOfAuthentication = tryToAuthenticateWithUsernameAndPassword(user);
		SecurityContextHolder.getContext().setAuthentication(resultOfAuthentication);
		httpResponse.setStatus(HttpServletResponse.SC_OK);
		TokenResponse tokenResponse = new TokenResponse(resultOfAuthentication.getDetails().toString());
		String tokenJsonResponse = new ObjectMapper().writeValueAsString(tokenResponse);
	...
	private Authentication tryToAuthenticateWithUsernameAndPassword(User user) {
		UsernamePasswordAuthenticationToken requestAuthentication = new UsernamePasswordAuthenticationToken(user.getUsername(), user.getPassword());
		return tryToAuthenticate(requestAuthentication);
	}
